...
- **사번** 기반 로그인 (`알파벳 1자 + 숫자 6자리`, 예: `D123456`)
...
- 다중 행 동시 편집 및 일괄 저장
- **일괄 저장 중 overlay 표시** — 저장 진행 중 입력값 변경 차단
- **재조회 버튼** — 일괄 저장 버튼 왼쪽에 위치, draft 있을 때 확인 후 재조회
- 목록 정렬: 팀명 오름차순 → 팀장 우선 → 이름 오름차순
- **사번 중복 사전 체크** (기존 참가자 + 현재 입력 행 간 교차 검사)
- **팀장 동시 교체** — 기존 팀장 off + 신규 팀장 on을 같은 일괄 저장으로 처리
...
|--------|----------|------|
| `이름` | **필수** | 참가자 이름 |
| `사번` | **필수** | 로그인 계정으로 사용, 중복 불가 (`알파벳 1자 + 숫자 6자리`) |
| `부서` | 선택 | 소속 부서 |
| `직급` | 선택 | 직급/직책 |
...
- **점수 입력 필드**: 포커스 시 0 제거, 블러 시 미입력이면 0으로 복원
- **팀 간 점수 독립 유지**: 한 팀 점수 입력이 다른 팀 저장 점수에 영향을 주지 않음
- 팀 개별 저장 후 다른 팀의 미저장 변경값 유지
...
- 팀 미배정 참가자: "아직 지정된 팀이 없습니다" 안내
- 결과 미공개 시: 공개 대기 안내
- 결과 공개 후 순위별 메시지 표시 (순위 카드 위에 위치)
  - 1~3등: 수상 등급별 축하 문구 (에메랄드 배경)
  - 4등 이상: 노고·성취감 위로 문구 (파랑 배경)

### 팀장 — 팀원 추가

- 팀원 카드에 이메일 대신 **부서 / 직급** 표시
